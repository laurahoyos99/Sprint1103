...
, CHURNFLAGRESULT AS (
SELECT DISTINCT FECHA_EXTRACCION,Mes,t.ACT_ACCT_CD,PFLAG,RangoVelocidad,PrecioMbp,MaxFecha,
CASE WHEN c.MaxFecha IS NOT NULL THEN "Churner"
WHEN c.Maxfecha IS NULL THEN "NonChurner" end as ChurnFlag
FROM TABLAPRELIMINAR t LEFT JOIN CHURNERSCRM  c ON  t.ACT_ACCT_CD=c.ACT_ACCT_CD 
AND DATE_TRUNC(MaxFecha, MONTH)=t.Mes
GROUP BY 1,2,3,4,5,6,7)

SELECT Mes,PFLAG, RangoVelocidad, ROUND(AVG(PrecioMbp),2) AS PrecioMbp, COUNT(DISTINCT ACT_ACCT_CD) AS Reg
FROM CHURNFLAGRESULT 
