...
, CRUCEPLANMINCHURN AS(
    SELECT DISTINCT c.CONTRATOCRM, EXTRACT(MONTH FROM  c.FechaChurn) as MESC--, EXTRACT(MONTH FROM c.PrimerChurn) AS MESMIN
...
SELECT MesC,PFLAG, RangoVelocidad, ROUND(AVG(PrecioMbp),2) AS PrecioMbp, COUNT(CONTRATOCRM) AS Reg
